# ResourceSteward (资源管家) 页面与功能功能规格清单
...
- 常见故障现象：列表显示“当前没有建议降级或退出的应用” → 开启了“只看建议”，当前运行应用全部处于前台或常用保护中，关掉开关即可查看全量；操作按钮置灰不可点击 → 该应用当前处于前台活跃状态；后台服务没有“退出”按钮 → 无普通 App 身份或 Bundle ID，不能通过系统应用接口请求退出
...
- 前置条件：存在待确认的手动操作，或 Level 0 下的 Jev 建议
...
- 前置条件：设置中开启 Level 1；执行无需通知权限，横幅需要通知权限
- 常见故障现象：执行了治理但未收到通知 → 系统通知权限未授权，或系统开启了“勿扰模式 / 专注模式”；目标应用未被自动处理 → 未满足候选或模型阈值，或处于 30 秒动作观察/90 秒应用冷却
...
1. [01. 菜单栏状态指示器 (Status Bar Item)](01-status-bar-item.md)
2. [02. 新手引导页 (Onboarding View)](02-onboarding-view.md)
3. [03. 资源看板与面板外壳 (Dashboard & Shell)](03-dashboard-header.md)
4. [04. 进程治理面板 (Process List Tab)](04-process-list.md)
5. [05. 常用保活管理面板 (Favorite Apps Tab)](05-favorite-apps.md)
6. [06. 设置与 Jev AI 配置面板 (Settings Tab)](06-settings-view.md)
7. [07. 浮动确认弹窗 (Confirm Prompt Panel)](07-confirm-panel.md)
8. [08. 系统通知与后台自动调度 (Reclaim Notifier)](08-notifications.md)

## Feature 文档

- [JevDecisionPipeline：候选筛选、两阶段判断与授权执行](JevDecisionPipeline/README.md)
- [ResourceDiagnostics：资源历史、决策证据与动作观测](ResourceDiagnostics/README.md)
